		...
		if( !is_wp_error( $request ) && $response_code == 200) {
			...
			if ( !is_wp_error( $body ) ) {
				...
				if (isset($data) && !empty($data) && !is_null($data)) {
					if (array_key_exists('valid',$data)) { $should_show_link = ($data['valid'] == false); } 
					if (array_key_exists('exists',$data)) { $proof_account_exists = $data['exists']; } 
				}
			}
		}
	} catch (Exception $e) {}
...
?>    
    <div class="form-group">
      ...
      <input class="proof-input-account" type="text" id="proof_account_id" value="<?=$options['account_id']?>" name="<?php echo $this->plugin_name; ?>[account_id]">
    </div>
    <div class="form-group">
      <label for="proof-factor[user_id]">User Id:</label>
      <input class="proof-input-account" type="text" id="proof_user_id" value="<?=$options['user_id']?>" name="<?php echo $this->plugin_name; ?>[user_id]">
    </div>
    <div class="form-group">
        <input type="submit" name="submit" id="submit" class="pf-button pf-button-sm pf-button-secondary" value="Update Proof Factor Credentials">
    </div>
	  <?php
		  if ($should_show_link) {
		   ?>
...
<hr>
  ...
  <p>In order to capture purchases from WooCommerce you will need to link WooCommerce with your Proof Factor Account!</p>
